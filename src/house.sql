...
alter table agencies add constraint phone_number check ( length(phone_number)=13 and phone_number ilike '+996%') ;
...
--1
select owners.first_name ,owners.last_name,houses.house_type  from houses inner join owners on houses.owner_id = owners.id where max(first_name)
--2
select houses.furniture from houses where houses.price between 1500 and 2000;
--3
select addresses.city, addresses.region,addresses.street, houses.house_type from houses inner join addresses on houses.address_id = addresses.id where addresses.id=5 or  addresses.id=6 or  addresses.id=7 or  addresses.id=8 or  addresses.id=9 or  addresses.id=10 group by addresses.city, addresses.region, addresses.street, houses.house_type;
--4
select houses.house_type,houses.price, houses.description,houses.room, customers.first_name,owners.first_name,agencies.name from rent_info inner join houses on rent_info.house_id = houses.id inner join customers  on customer_id = rent_info.customer_id inner join owners on rent_info.owner_id = owners.id inner join addresses on houses.address_id = addresses.id inner join agencies on rent_info.agency_id = agencies.id group by houses.house_type, houses.price, houses.description, houses.room, customers.first_name, owners.first_name, agencies.name;
--5
select * from customers where date_of_birth li